...
lockTrace :: String -> EmulatorTrace ()
lockTrace secretWord = do
    let w1 = Wallet 1
    hdl <- Trace.activateContractWallet w1 (lock @ContractError)
    Trace.callEndpoint @"lock" hdl (LockParams secretWord (Ada.lovelaceValueOf 10))
    void $ Trace.waitNSlots 1

guessTrace :: String -> EmulatorTrace ()
guessTrace guessWord = do
    let w2 = Wallet 2
    hdl <- Trace.activateContractWallet w2 (guess @ContractError)
