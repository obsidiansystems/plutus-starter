...
create :: Uniswap -> CreateParams -> Contract w s Text Tx
...
close :: Uniswap -> CloseParams -> Contract w s Text Tx
...
remove :: Uniswap -> RemoveParams -> Contract w s Text Tx
...
add :: Uniswap -> AddParams -> Contract w s Text Tx
add us AddParams{..} = do
    pkh                           <- pubKeyHash <$> ownPubKey
...
swap :: Uniswap -> SwapParams -> Contract w s Text (Tx, Integer) -- (Tx, ScriptSize)
swap us SwapParams{..} = do
    ...
    let txIns :: [TxInType] = catMaybes $ map txInType $ Set.toList $ txInputs ledgerTx
