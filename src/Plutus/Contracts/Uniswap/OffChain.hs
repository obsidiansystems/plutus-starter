...
import           Prelude                          as Haskell (Semigroup (..), String, Int, dropWhile, flip, show, (^), div)
...
type UniswapUserSchema = Endpoint "create" CreateParams
...
create :: forall w s. Uniswap -> CreateParams -> Contract w s Text CardanoTx
...
close :: forall w s. Uniswap -> CloseParams -> Contract w s Text CardanoTx
...
remove :: forall w s. Uniswap -> RemoveParams -> Contract w s Text CardanoTx
...
add :: forall w s. Uniswap -> AddParams -> Contract w s Text CardanoTx
add us AddParams{..} = do
    pkh                           <- Contract.ownPubKeyHash
...
swap :: forall w s. Uniswap -> SwapParams -> Contract w s Text (CardanoTx, Integer) -- (Tx, ScriptSize)
swap us SwapParams{..} = do
    ...
    logInfo $ show tx
    ledgerCardanoTx :: CardanoTx <- submitTxConstraintsWith lookups tx
    logInfo $ show ledgerCardanoTx
    void $ awaitTxConfirmed $ getCardanoTxId ledgerCardanoTx
    logInfo $ "swapped with: " ++ show lp
    let txIns :: [TxInType] = case ledgerCardanoTx of
          Right ledgerTx -> catMaybes $ map txInType $ Set.toList $ txInputs ledgerTx
          Left _ -> [] -- TODO: Add some logic here to handle cardano api tx proper
        ...
        txInScriptSize txInType  = case txInType of
          ...
          _ -> 0 -- TODO: pattern match for all ConsumeScriptAddress types
        scrSize = sum $ map txInScriptSize txIns
    -- return Tx and script size
    return (ledgerCardanoTx, scrSize)
...
findUniswapInstance :: forall a b w s. Uniswap -> Coin b -> (UniswapDatum -> Maybe a) -> Contract w s Text (TxOutRef, ChainIndexTxOut, a)
